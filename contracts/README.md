This contract implements the registration and pay-to-subscribe flow of the device.

## Compile
```truffle compile```

## Test
```truffle test```

## Testnet

### Deploy
```ioctl contract compile Prophecy --abi-out a.abi --bin-out a.bin```

```ioctl contract deploy bin a.bin --endpoint api.testnet.iotex.one:443```

### Invoke
We use `io17200hf5gkal298yp2fygme2h5thl7hraafu5je` deployed on testnet (via this [action](testnet.iotexscan.io/action/5235049db5c4550ca31ea726be30d518f7fcfef8a53b23b5cd5225f11e40a40e)) as an example to invoke.

**setRegistrationFee()**
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi setRegistrationFee --with-arguments '{"fee":"1"}' --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash: testnet.iotexscan.io/action/1ec24eff9325997e5db149b31882676bbca1f2b8daaf157136dd217c32ffe054
```

**registrationFee()**
```
ioctl contract test function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi registrationFee --endpoint api.testnet.iotex.one:443
return: 1
```

**setSubscriptionFee()**
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi setSubscriptionFee --with-arguments '{"fee":"1"}' --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash: testnet.iotexscan.io/action/b9587a00d06d69f0210469f1ab7627dab0d8e800649b8c6b748a4d2ce9574e85
```

**subscriptionFee()**
```
ioctl contract test function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi subscriptionFee --endpoint api.testnet.iotex.one:443
return: 1
```

**preRegisterDevice()**
Assume deivce id is 352656100794612
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi preRegisterDevice --with-arguments '{"_deviceIdHash":"0x87b94e1b4114ed2746e032605265993b811798401bee23d749a8f3c61f0ae16a"}' --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash: testnet.iotexscan.io/action/8d72d2843a8b36ac644e58c8259f9f7485fa514204d6cf970e954415a273cd5e
```

**registerDevice()**
Assume deivce id is 352656100794612
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi registerDevice --with-arguments '{"_deviceId":"0x0000000000000000000000000000000000333532363536313030373834343135","_freq":"30","_price":"1","_spec":"trypebble.io/123","_rsaPubkeyN":"","_rsaPubkeyE":"0x010001"}' -l 10000000 --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash: testnet.iotexscan.io/action/ffec6719c510bec3bd3e68df4e594bb318fb5a40b875ef653c913707970de32a
```

**updateDevice()**
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi updateDevice --with-arguments '{"_deviceId":"0x0000000000000000000000000000000000333532363536313030373834343135","_freq":"30","_price":"1","_spec":"trypebble.io/123","_rsaPubkeyN":"","_rsaPubkeyE":"0x010001"}' -l 10000000 --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash:iotexscan.io/action/db36a5b8452e35102a34c844b30e13655d5fcde2ed3903967dd6aa8d72879459
```
**getDeviceInfoByID()**
```
ioctl contract test function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi getDeviceInfoByID --with-arguments '{"_deviceId":"0x0000000000000000000000000000000000333532363536313030373934363132"}' --endpoint api.testnet.iotex.one:443
return: 
```

**subscribe()**
Assume deivce id is 352656100794612, subscribe for 4 blocks
```
ioctl contract invoke function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi subscribe 10 --with-arguments '{"_deviceId":"0x0000000000000000000000000000000000333532363536313030373934363132","_duration":"4","_storageEPoint":"","_storageToken":""}' -l 10000000 --endpoint api.testnet.iotex.one:443
...
Wait for several seconds and query this action by hash: testnet.iotexscan.io/action/f7626ce6784cbbcb80cef065d3bb2a0ba9870d4e735dbe3e202e7a75b19531b5
```

**getDeviceOrderByID()**
```
ioctl contract test function io17200hf5gkal298yp2fygme2h5thl7hraafu5je a.abi getDeviceOrderByID --with-arguments '{"_deviceId":"0x0000000000000000000000000000000000333532363536313030373934363132"}' --endpoint api.testnet.iotex.one:443
return:  
```

## Mainnet

### Deploy
```ioctl contract compile Prophecy --abi-out a.abi --bin-out a.bin```
``` ioctl contract deploy bin a.bin --endpoint api.iotex.one:443```

Contract `io1026xsfcr63pyz5njemkzlsfl8yd9myzp6dyg4l` has been deployed on mainnet (via this [action](iotexscan.io/action/0915c2e463883cb393b2240cbf5445f25ffd1fce33f70ddfcb425534d92f7eb4
))
